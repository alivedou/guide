...
window.requireNewPasswordAdmin = (message) => {
    return new Promise((resolve) => {
        ...
        titleEl.innerText = "管理员设置临时密码";
        msgEl.innerHTML = message || "请输入临时密码（有效期30分钟）：<br><small style='color:#e67e22'>注意：这是临时密码，用户登录后需立即修改</small>";
        passInput.value = '';
        passInput.type = 'text'; // 管理员重置密码时应显示明文，方便管理员记录与核对
        passInput.placeholder = "请输入临时密码...";
        modal.style.display = 'flex';
...
window.resetUserPasswordAdmin = async (userId) => {
    ...
    await SyncUI.perform('USER_MANAGE', async () => {
        ...
        showToast("临时密码已设置（有效期30分钟）", "#2ecc71");
    });
};
