...
// 获取近 7 日内点击数据，并自动过滤清理过期历史记录
const getFrequentItemsData = () => {
    const clicksHistory = JSON.parse(localStorage.getItem('nav_clicks_history') || '{}');
    const now = Date.now();
    const sevenDaysMs = 7 * 24 * 60 * 60 * 1000;
    const filteredHistory = {};
    const freqCounts = {};

    Object.keys(clicksHistory).forEach(id => {
        if (Array.isArray(clicksHistory[id])) {
            const validClicks = clicksHistory[id].filter(ts => (now - ts) < sevenDaysMs);
            if (validClicks.length > 0) {
                filteredHistory[id] = validClicks;
                freqCounts[id] = validClicks.length;
            }
        }
    });

    localStorage.setItem('nav_clicks_history', JSON.stringify(filteredHistory));
    return freqCounts;
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 全局监听卡片点击 (支持常去网站 7 天点击次数统计)
    document.getElementById('grid-container').addEventListener('click', (e) => {
        const card = e.target.closest('.card');
        const videoCard = e.target.closest('.video-card');
        const targetCard = card || videoCard;
        const link = e.target.closest('a');
        
        if (targetCard && link && !targetCard.classList.contains('card-add-new') && !e.target.closest('.admin-actions')) {
            ...
            if (id) {
                ...
                // 记录包含时间戳的 7 天点击历史
                let clicksHistory = JSON.parse(localStorage.getItem('nav_clicks_history') || '{}');
                if (!Array.isArray(clicksHistory[id])) {
                    clicksHistory[id] = [];
                }
                clicksHistory[id].push(Date.now());
                localStorage.setItem('nav_clicks_history', JSON.stringify(clicksHistory));
            }
        }
    });
    ...
    // ==================== 全局高优先级快捷键系统 (Esc 一键复位 / Ctrl+K 极速聚焦) ====================
    document.addEventListener('keydown', (e) => {
        const seaInput = document.getElementById('sea-input');
        
        // 1. Ctrl/Cmd + K 极速接管并清空
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'k') {
            e.preventDefault();
            if (seaInput) {
                // 如果在极简沉浸模式未展开状态下，强制展开并重置
                if (document.body.classList.contains('zen-active') && !isZenTempExpanded) {
                    expandZen();
                }
                seaInput.value = '';
                executeLocalSearch(''); // 触发空值状态
                seaInput.focus();
                showToast('已接管焦点，输入框已清空');
            }
            return;
        }
        ...
        if (e.key === 'Escape') {
            ...
            if (overlayEdit) {
                if (overlayEdit.classList.contains('open') && typeof closeModal === 'function') {
                    closeModal();
                } else {
            ...
            if (overlaySidebar && overlaySidebar.classList.contains('visible')) {
                overlaySidebar.classList.remove('visible');
            }

            const engineList = document.getElementById('engine-list');
            if (engineList && engineList.classList.contains('show')) {
                engineList.classList.remove('show');
            }

            // b. 收拢并关闭联想下拉菜单
            const seaDropdown = document.getElementById('sea-dropdown');
            if (seaDropdown) seaDropdown.style.display = 'none';

            // c. 极简模式下，退回到未展开的初始极简纯净态
            if (document.body.classList.contains('zen-active') && isZenTempExpanded) {
                isZenTempExpanded = false;
                renderNav();
                setTimeout(() => {
                    const finalInput = document.getElementById('sea-input');
                    if (finalInput) {
                        finalInput.value = '';
                        finalInput.focus();
                    }
    ...
    document.addEventListener('keydown', (e) => {
        ...
        // 如果当前按键是 ESC 且搜索面板开着，由专门的键盘事件处理
        ...
        if (!isFocusInput && (e.key.length === 1 || e.key === 'Backspace')) {
            ...
            if (document.body.classList.contains('zen-active') && !isZenTempExpanded) {
                ...
                setTimeout(() => {
                    ...
                    if (finalInput) {
                        if (e.key === 'Backspace') {
                            finalInput.value = finalInput.value.slice(0, -1);
                        } else {
                            finalInput.value += e.key;
                        }
                        // 手动触发模糊过滤联想
                        executeLocalSearch(finalInput.value);
                    }
                }, 120);
            } else {
                seaInput.focus();
...
const initZenMode = () => {
    ...
    document.addEventListener('touchend', (e) => {
        ...
        // 水平向右滑动距离超过 80 像素且垂直偏离不超过 40 像素时唤出侧边栏
...
// 切换极简沉浸模式 (Zen Mode) 与 标准模式 (Standard Mode)
const toggleZenMode = () => {
    if (!appData.settings) appData.settings = {};
    appData.settings.zenMode = !appData.settings.zenMode;
    isZenTempExpanded = false; // 切换模式时重置临时展开状态
    
    if (typeof saveAll === 'function') {
        saveAll(false);
    }
    renderNav();
    showToast(appData.settings.zenMode ? '已开启极简沉浸模式' : '已切换至标准模式');
};
...
const renderNav = () => {
    ...
    const clickData = getFrequentItemsData(); // 升级：获取近 7 日内有点击的历史统计
    const freqItemIds = Object.keys(clickData).filter(id => clickData[id] >= 10); // 升级：近 7 日点击累计达 10 次及以上才归入常去
    const hasFrequent = freqItemIds.length > 0;
    ...
    // 连续滚动：渲染所有分类区块
    cats.forEach((cat) => {
        // 如果处于极简沉浸模式且已展开，严格遵循单一视图原则，仅渲染和展示当前选中分类，屏蔽其他分类内容干扰
        const isZenModeSetting = appData.settings && appData.settings.zenMode;
        if (isZenModeSetting && isZenTempExpanded && cat.id !== activeCatId) {
            return;
        }
...
const openItemEdit = (id, catId) => {
    ...
    // 智能魔法棒图标一键获取 (对齐4.4规范：根目录 favicon -> 配置图标库 -> 首字母高亮替代)
    document.getElementById('btn-icon-magic').addEventListener('click', () => {
        ...
        try {
            ...
            // 优先级 1：优先测试并获取多个主流根目录及Favicon接口
            const favUrls = [
                `${origin}/favicon.ico`,
            ...
            // Promise 形式测试加载
            const testFavicon = (favUrl) => {
                return new Promise((resolve, reject) => {
                    const img = new Image();
                    img.src = favUrl;
                    img.onload = () => resolve(favUrl);
                    img.onerror = () => reject();
                });
            };

            showToast("正在智能抓取最优图标...");

            // 挑选最快成功的一个 favicon
            Promise.any(favUrls.map(testFavicon)).then((fastestFav) => {
                if (!resolved) {
                    resolved = true;
                    if (iconInput) {
                        iconInput.value = fastestFav;
                        updatePreview(fastestFav);
                        showToast(`已智能抓取根目录 Favicon 图标`);
                    }
                }
            }).catch(() => {
                if (resolved) return;
                
                // 优先级 2：若抓取失败，自动根据网站关键词匹配配置图标库(EMOJI_KEYWORDS)
                let matchedEmoji = '';
                const fullText = (title + ' ' + domain).toLowerCase();
                ...
                if (matchedEmoji && iconInput) {
                    resolved = true;
                    iconInput.value = matchedEmoji;
                    updatePreview(matchedEmoji);
                    showToast(`已自动匹配配置图标库 Emoji: ${matchedEmoji}`);
                } else {
                    // 优先级 3：若均无法匹配，退回采用首字母加纯色高亮替代作为兜底
                    if (resolved) return;
                    resolved = true;
                    
                    const fallbackChar = title ? title.trim().charAt(0).toUpperCase() : domain.charAt(0).toUpperCase();
                    if (iconInput) {
                        iconInput.value = fallbackChar;
                        updatePreview(fallbackChar);
                        showToast(`已应用首字母 [${fallbackChar}] 高亮替代兜底`);
                    }
                }
            });

        } catch (err) {
            showToast('URL 格式不合法，无法抓取', '#e74c3c');
        }
    });
...
const exportConfig = () => {
    ...
    // 导出文件命名为 用户名_月日.json (多用户系统开发前，默认使用 admin/guest 作为用户名)
    const now = new Date();
    const month = String(now.getMonth() + 1).padStart(2, '0');
    const day = String(now.getDate()).padStart(2, '0');
    const userName = isAdmin ? 'admin' : 'guest';
    
    a.href = url;
    a.download = `${userName}_${month}${day}.json`;
    a.click();
...
const initSearch = () => {
    ...
    // 引擎顺流轮替顺序支持阵：Bing -> Google -> Baidu -> Sogou 并回到 Bing
    const enginesSequence = [
        { id: 'bing', action: 'https://cn.bing.com/search?q=' },
        { id: 'google', action: 'https://www.google.com/search?q=' },
        { id: 'baidu', action: 'https://www.baidu.com/s?wd=' },
        { id: 'sogou', action: 'https://www.sogou.com/web?query=' }
    ];

    const rotateEngine = () => {
        let currentIndex = enginesSequence.findIndex(e => e.id === currentEngine);
        let nextIndex = (currentIndex + 1) % enginesSequence.length;
        const nextEngine = enginesSequence[nextIndex];

        currentEngine = nextEngine.id;
        currentEnginePrefix = nextEngine.action;
        ...
        updateEngineTriggerUI(currentEngine);
        showToast(`搜索引擎切换至: ${currentEngine.toUpperCase()}`);
    };

    // 1. 搜索引擎切换逻辑
    if (engineTrigger) {
        // 单击引擎图标：直接无断点循环切换搜索引擎，极致节省场景切换成本
        ...
        // 鼠标右键或长按：弹起完整选择列表（兼容下拉快速定位）
        engineTrigger.addEventListener('contextmenu', (e) => {
            e.preventDefault();
            e.stopPropagation();
    ...
    seaInput.addEventListener('keydown', (e) => {
        ...
        const queryVal = seaInput.value.trim();

        // 历史搜索状态回拨反溯（输入栏为空时上下翻卷历史请求记录）
        if (!queryVal) {
            ...
            if (history.length > 0) {
                if (e.key === 'ArrowUp') {
                    ...
                    seaInput.value = history[historyIndex];
                    executeLocalSearch(seaInput.value);
                    return;
                }
                if (e.key === 'ArrowDown') {
                    ...
                    seaInput.value = historyIndex === -1 ? '' : history[historyIndex];
                    executeLocalSearch(seaInput.value);
                    return;
                }
            }
        } else {
            historyIndex = -1; // 输入框不为空时重置历史索引
        }
        ...
        if (e.key === 'Enter') {
            const query = seaInput.value.trim();
            if (query) {
                saveSearchHistory(query); // 记录搜索词到历史中
                window.open(currentEnginePrefix + encodeURIComponent(query), '_blank');
            }
        }
    });
