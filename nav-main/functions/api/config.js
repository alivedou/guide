...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (settings) {
      ...
      writeQueries.push(env.DB.prepare('UPDATE user_settings SET card_width = ?, zen_mode = ?, show_frequent = ?, bg_url = ?, simple_mode = ?, link_target = ?, theme_mode = ?, sync_interval = ? WHERE user_id = ?')
        .bind(settings.cardWidth !== undefined ? settings.cardWidth : null, settings.zenMode ? 1 : 0, settings.showFrequent ? 1 : 0, settings.bgUrl || null, settings.simpleMode ? 1 : 0, linkTarget, settings.themeMode || 'auto', settings.syncInterval !== undefined ? settings.syncInterval : 7, userId));
    }
...
export async function onRequestDelete(context) {
  ...
  try {
    ...
    const queries = [
      env.DB.prepare('DELETE FROM categories WHERE user_id = ?').bind(userId),
      env.DB.prepare('DELETE FROM items WHERE user_id = ?').bind(userId),
      env.DB.prepare('UPDATE user_settings SET card_width = 125, zen_mode = 1, show_frequent = 1, bg_url = NULL, simple_mode = 0, link_target = \'_blank\', theme_mode = \'auto\', sync_interval = 7 WHERE user_id = ?').bind(userId)
    ];
