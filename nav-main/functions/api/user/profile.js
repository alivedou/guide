...
export async function onRequestGet(context) {
  ...
  if (!authUser.id) {
    return new Response(JSON.stringify({ error: "Unauthorized", code: "ERR_UNAUTHORIZED" }), { status: 401 });
  }
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // 2. 获取原用户信息
    const user = await env.DB.prepare('SELECT password_hash FROM users WHERE id = ?').bind(authUser.id).first();
    if (!user) {
    ...
    if (newPassword && newPassword.trim()) {
      ...
      if (user.password_hash !== oldHash) {
        return new Response(JSON.stringify({ error: "原密码验证失败" }), { status: 401 });
      }
    }
