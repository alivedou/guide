...
export async function onRequestPost(context) {
  ...
  try {
    ...
    initQueries.push(env.DB.prepare('UPDATE user_settings SET card_width = ?, zen_mode = ?, link_target = ?, sync_interval = ? WHERE user_id = ?')
      .bind(settings.cardWidth || 125, settings.zenMode ? 1 : 0, linkTarget, settings.syncInterval !== undefined ? settings.syncInterval : 7, uuid));
