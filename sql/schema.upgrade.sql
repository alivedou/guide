-- 路径: sql/schema.upgrade.sql（Docker 不打包本文件；老库手动升级用）
-- 增量补丁：给「已经建过旧表」的 D1 补齐缺列/缺表
-- CREATE TABLE IF NOT EXISTS 不会升级已有表结构，老库必须跑本文件
-- 每条 ALTER 若列已存在会报 duplicate column，可忽略继续下一条
