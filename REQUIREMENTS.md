...
#### 4.3.4 高危操作敏感校验 (Double Password Check)

- 任何管理员在管理控制面板执行诸如“冻结/解冻账户”、“重置特定用户登录密码”以及“删除某一注册用户及其所有云端数据”等高危、敏感行为时，系统**强制弹出二次验证框**，要求当前操作的管理员必须二次输入自己的账户密码（由 [nav-main/functions/api/admin/users.js](nav-main/functions/api/admin/users.js#L81) 后端对密文再次进行 SHA-256 对称验证）。验证通过方可下发 D1 事务。
