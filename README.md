...
4. **配置环境变量**：
   ...
   *    **可选项**（用于多人使用时管理网站用）：
    *    **`TELEGRAM_BOT_TOKEN`**：用于日常管理异常通知 (可选项：用于管理员定时日报和异常项目的通知，chat_id的话是在网站用户个人资料自己填写的，所以这里chat_id不用加到环境变量)。
    *    **`ON_SECRET`**：一长串安全随机字符串 (可选项：用于管理员定时日报的安全拦截，个人使用的就不需要日报了,主要是管理注册异常用户的操作通知)。
     
**ps：** 如果需要多人使用需要日报的话，要到系统异常告警与审计通知配置及测试指南下的第3个步骤操作。
