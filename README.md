...
### 🛠️ 相比原版增加的功能与重磅升级：
- **本地自定义背景升级（IndexedDB 10MB 大空间）**：本地上传图片限制由 3MB 大幅放宽至 **10MB**，底层存储引擎由 `localStorage` 重构升级为支持大文件的 **`IndexedDB`** 数据库，彻底消除了 5MB 网页存储溢出的崩溃风险。支持首屏预加载、老缓存无缝向后自动兼容与迁移，保证开屏渲染零闪烁。
- **本地调试配额全景对齐 (server.js 重构)**：本地开发环境 `server.js` 完美移植生产环境的 `QUOTA_CONFIG` 与 `getQuota` 动态配额分发逻辑，通过 SQLite 数据库关联动态鉴权，完美模拟了和生产环境、需求文档一致的权限限制矩阵（如管理员分类上限 100/网址上限 500，不再被硬编码死锁），开发调试 100% 对齐。
- **代码著作权保护与规范**：项目内所有的核心 js 服务、Serverless 路由及逻辑脚本顶部均添加了统一作者 `adou` 和 MIT 免责声明规范，代码风格严谨，著作版权清晰。
- **增加搜索框**：搜索框可选内置多个主流搜索引擎，并支持站内书签卡片的实时快速模糊检索。
- **极简沉浸模式**：深度优化的禅意沉浸体验，聚焦式 Spotlight 搜索框，全键盘友好响应。
- **返回顶部/底部**：增加高颜值悬浮快捷按钮，避免长页面滑动疲劳。
- **自定义跳转**：支持设置链接在当前页或新页面自适应打开。
- **图标自愈与智能魔棒**：稳健的 6 级降级逻辑保障图标极致可用；编辑器内一键抓取并自动推荐最优网络/CDN 图标。
...
4. **配置环境变量**：
   * 在 Pages 项目页面，点击 **设置 (Settings)** -> **环境变量** 下点击 **“添加变量”** (需同时添加 Production 和 Preview)：
     *   **`TOKEN`**：您的首席管理员登录密码 (支持明文或 SHA-256 哈希)。
     *   **`JWT_SECRET`**：一长串安全随机字符串 (用于 JWT Token 的加解密签名)。
     *   **`CRON_SECRET`**：一长串安全随机字符串 (用于管理员定时日报的安全拦截)。
5. 点击保存。

### 第四步：初始化 D1 数据库表结构 (D1 Migrations)
系统基于多用户 D1 关系型 SQLite 运作，在首次运行前必须进行 D1 数据库的表迁移和初始化：
1. 本地安装依赖：在您的本地工程目录下运行 `npm install`。
2. 登录 Cloudflare 授权：运行 `npx wrangler login`，根据浏览器弹窗提示完成登录。
