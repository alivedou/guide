...
| **契约测试** | `npm test` | 基线 HTTP 契约 + 阶段 0 卫生（`tests/`） |
...
复制根目录 `.env.example` 为 `.env`，填写 **`JWT_SECRET`**（登录签名，必填）。旧版单用户 `TOKEN` / `kv_mock.json` 已废弃，可忽略。程序默认在 `http://localhost:3000` 启动，数据在 `local_d1.db` 与 `local_kv/`。
