...
-- ==========================================
-- 💡 小白新手快速指引：
-- 1. [网页一键执行]：您可以直接复制本文件全部内容，粘贴到 Cloudflare Dashboard 
--    (Workers & Pages -> D1 -> 选中您的数据库 -> 点击 Console 选项卡) 中直接执行。
-- 2. [本地终端部署]：也可以在根目录下直接执行：
--    wrangler d1 execute cloudnav-db --remote --file=./schema.sql
-- 3. [超级管理员账号激活]：初始化数据库后，请立即在前端网页注册第一个账号。
--    系统内置“首位用户自动设为管理员”的智能安全机制，首个注册用户将直接成为超级管理员，无需手动配置！
-- ==========================================
